...
module edb_top (
    ...
    output bscan_TDO,
    input  DPLLTest_clk,
    input  [0:0] DPLLTest_PrbsError,
    input  [0:0] DPLLTest_PrbsRight,
    output [0:0] DPLLTest_LedCntRst,
    output [0:0] DPLLTest_ControlMiiLoop,
    input  Mcst2MII_clk,
    input  [1:0] Mcst2MII_TxMcstData,
    input  [0:0] Mcst2MII_MiiTxCEn,
    input  [2:0] Mcst2MII_MiiTxData,
    input  [0:0] Mcst2MII_MiiTxEn,
    input  [0:0] Mcst2MII_MiiRxCEn,
    input  [3:0] Mcst2MII_MiiRxData,
    input  [0:0] Mcst2MII_MiiRxDV,
    input  [0:0] Mcst2MII_PrbsError
);
    ...
    wire DPLLTest_module_select;
    wire DPLLTest_module_inhibit;
    wire DPLLTest_module_tdo;

    wire Mcst2MII_module_select;
    wire Mcst2MII_module_inhibit;
    wire Mcst2MII_module_tdo;

    assign DPLLTest_module_select       = edb_module_selects[0];
    assign edb_module_inhibit[0]    = DPLLTest_module_inhibit;
    assign edb_module_tdo[0]        = DPLLTest_module_tdo;
    assign Mcst2MII_module_select       = edb_module_selects[1];
    assign edb_module_inhibit[1]    = Mcst2MII_module_inhibit;
    assign edb_module_tdo[1]        = Mcst2MII_module_tdo;
    ...
    edb_vio_top #(
        ...
        .edb_module_select  ( DPLLTest_module_select  ),
        .edb_module_inhibit ( DPLLTest_module_inhibit ),
        .edb_module_tdo     ( DPLLTest_module_tdo ),
        .clk                ( DPLLTest_clk ),
        .probe_in0          ( DPLLTest_PrbsError ),
        .probe_in1          ( DPLLTest_PrbsRight ),
        .probe_in2          ( 1'b0 ),
        .probe_in3          ( 1'b0 ),
        .probe_in4          ( 1'b0 ),
        .probe_in5          ( 1'b0 ),
        ...
        .probe_in62         ( 1'b0 ),
        .probe_in63         ( 1'b0 ),
        .probe_out0         ( DPLLTest_LedCntRst ),
        .probe_out1         ( DPLLTest_ControlMiiLoop ),
        .probe_out2         (  ),
        .probe_out3         (  ),
        .probe_out4         (  ),
        .probe_out5         (  ),
    ...
    edb_la_top #(
        ...
        .bscan_UPDATE           ( bscan_UPDATE  ),
        .edb_user_dr            ( edb_user_dr ),
        .edb_module_select      ( Mcst2MII_module_select  ),
        .edb_module_inhibit     ( Mcst2MII_module_inhibit ),
        .edb_module_tdo         ( Mcst2MII_module_tdo ),
        .clk                    ( Mcst2MII_clk ),
        .trig_in                ( 1'b0 ),
        .trig_in_ack            (  ),
        .trig_out               (  ),
        .trig_out_ack           ( 1'b0 ),
        .probe0                 ( Mcst2MII_TxMcstData ),
        .probe1                 ( Mcst2MII_MiiTxCEn ),
        .probe2                 ( Mcst2MII_MiiTxData ),
        .probe3                 ( Mcst2MII_MiiTxEn ),
        .probe4                 ( Mcst2MII_MiiRxCEn ),
        .probe5                 ( Mcst2MII_MiiRxData ),
        .probe6                 ( Mcst2MII_MiiRxDV ),
        .probe7                 ( Mcst2MII_PrbsError )
    );
...
module edb_la_top #(
    ...
    localparam BYTES_PER_WORD = (CAPTURE_WIDTH-1)/8 + 1;
    ...
    // tu_pattern | trig_pos | stop_trig | run_trig_imdt | run_trig | cstate
    // 2-bit        17-bit     1-bit       1-bit           1-bit      3-bit
    localparam R0_WIDTH = 8 + BUF_MAX_ADDR_W;
    ...
    assign internal_reg_r0[3]                    = la_run_trig;
    assign internal_reg_r0[4]                    = la_run_trig_imdt;
    assign internal_reg_r0[5]                    = la_stop_trig;
    assign internal_reg_r0[6 +: BUF_MAX_ADDR_W]   = la_trig_pos;
    assign internal_reg_r0[6+BUF_MAX_ADDR_W +: 2] = la_trig_pattern;
    ...
    always @(posedge bscan_TCK or posedge bscan_RESET) begin
        if (bscan_RESET) begin
            ...
            la_run_trig      <= edb_user_dr[REG_MSB -2 - BUF_MAX_ADDR_W -2];
            la_run_trig_imdt <= edb_user_dr[REG_MSB -2 - BUF_MAX_ADDR_W -1];
            la_stop_trig     <= edb_user_dr[REG_MSB -2 - BUF_MAX_ADDR_W];
            la_trig_pos      <= edb_user_dr[REG_MSB -2 -: BUF_MAX_ADDR_W];
            la_trig_pattern  <= edb_user_dr[REG_MSB -: 2];
        end
    end
    ...
    assign data_to_addr_counter = (addr_sel) ? incremented_address[31:0] : address_in;
    //assign incremented_address  = address_counter + word_size_bytes;
    assign incremented_address  = (address_counter[0 +: 15] == BYTES_PER_WORD-1) ? 
    ...
    generate
        for (i = 0; i < NUM_PROBES; i = i + 1) begin : GEN_PROBE
            ...
            if (PROBE_TYPE == 1 || PROBE_TYPE == 2) begin
                //assign cap_fifo_din[CAP_LSB +: PROBE_WIDTH] = probes[i][0 +: PROBE_WIDTH];
                assign cap_fifo_din[CAP_LSB +: PROBE_WIDTH] = this_probe_p2;
            end

            if (PROBE_TYPE == 1 || PROBE_TYPE == 3) begin
                wire [(REG_PER_PW*REG_WIDTH)-1:0] probe_compared, probe_mask;
                wire [2:0] probe_pattern;
                wire       probe_cout;

                for (j = 0; j < REG_DEPTH; j = j + 1) begin : GEN_REG
                    localparam REG_ADDR = (1 + i)*REG_DEPTH + j; // addr mapping

                    if (j == 0) begin
                        reg [2:0] internal_reg_pr;
                        ...
                        always @(posedge bscan_TCK or posedge bscan_RESET) begin
                            if (bscan_RESET) begin
                                ...
                                internal_reg_pr <= edb_user_dr[0 +: 3];
                            end
                        end
                    ...
                    else if (j < REG_PER_PW*2+1) begin
                        reg [REG_WIDTH-1:0] internal_reg_pr;
                        ...
                        always @(posedge bscan_TCK or posedge bscan_RESET) begin
                            if (bscan_RESET) begin
                                ...
                                internal_reg_pr <= edb_user_dr[0 +: REG_WIDTH];
                            end
                        end

                        assign register_conn[REG_ADDR] = internal_reg_pr;

                        if (j < REG_PER_PW + 1) begin
...
module fifo_with_read #(
    ...
    assign pointer_eq = (wr_pointer[ADDR_WIDTH-1:0] == rd_pointer[ADDR_WIDTH-1:0]);
    assign pntr_msb_xor = wr_pointer[ADDR_WIDTH] ^ rd_pointer[ADDR_WIDTH];
    assign full  = pntr_msb_xor & pointer_eq;
    assign empty = ~pntr_msb_xor & pointer_eq;
    assign we = push & ~full;
    assign re = pop & ~empty;
    ...
    assign rd_addr = rd_pointer[ADDR_WIDTH-1:0] + raddr;
    assign phy_addr = rd_mode ? rd_addr : rd_pointer[ADDR_WIDTH-1:0];
...
module la_biu #(
    ...
    localparam LA_IDLE      = 3'h0;
    localparam LA_PRE_TRIG  = 3'h1;
    localparam LA_WAIT_TRIG = 3'h2;
    localparam LA_POST_TRIG = 3'h3;
    localparam LA_FULL      = 3'h4;
    ...
    reg  [BUF_MAX_ADDR_W-1:0] pos_counter;
    wire [BUF_MAX_ADDR_W:0] pos_counter_add1;
    wire trig_pos_reached;
    reg  counter_en;
    wire fifo_full;
    reg  fifo_push, fifo_pop;
    reg  read_mode;
    wire [CAPTURE_WIDTH-1:0] fifo_dout;
    wire fifo_rstn;
    ...
    //wire [$clog2(DATA_DEPTH)-1:0] row_addr; 
    wire [14:0] col_addr;
    ...
    assign pos_counter_add1 = pos_counter + 1;

    // To fix WAIT state passing the matched cycle of data to pre-trig
    // wait for 1 more during pre-trig
    //assign trig_pos_reached = (pos_counter_add1 == la_trig_pos);
    ...
    always @(posedge clk or negedge la_resetn) begin
        ...
        else if (cap_buf_read_done_p2) begin
            pos_counter <= 0;
        end 
        else if (counter_en) begin
            pos_counter <= pos_counter_add1[BUF_MAX_ADDR_W-1:0];
        end
    end
    ...
    always @(*) begin
        case(curr_state)
            ...
            LA_WAIT_TRIG: begin
                if (tu_trigger || run_trig_imdt_p2)
                    next_state = LA_POST_TRIG;
                else if (la_stop_trig)
                    next_state = LA_FULL;
                else
                    next_state = LA_WAIT_TRIG;
            end
...
module trigger_unit #(
    ...
    parameter TRIGIN_EN = 0,
    parameter PIPE = 0,
    parameter EMPTY_MASK = 0 /* Output High / Low when mask_in is all zero,
